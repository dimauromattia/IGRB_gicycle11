...
Cosmic $\gamma$ rays are associated with the most extreme phenomena and sources and, not surprisingly, measurements in this part of the electromagnetic spectrum have triggered huge interest and are the goal of several current and future experiments.
The {\it Fermi}-LAT
%\footnote{{\it Fermi}-LAT \url{https://fermi.gsfc.nasa.gov}, H.E.S.S. \url{https://www.mpi-hd.mpg.de/hfm/HESS/}, VERITAS \url{https://veritas.sao.arizona.edu}, HAWC \url{https://www.hawc-observatory.org} and CTA \url{https://www.cta-observatory.org}.} 
has revolutionized our understanding of the $\gamma$-ray sky at GeV energies while Imagining Atmospheric Cherenkov Telescopes (IACTs) (e.g., H.E.S.S., VERITAS and HAWC) cover the TeV band.\FIXME{I don't think you really need this for a GI panel.}
...
truly diffuse background.
The Extragalactic $\gamma$-ray background (EGB) is the sum of resolved extragalactic sources and
the IGRB. The IGRB was first measured a few decades ago but its exact composition is still a mystery.
...
Finally, Star Forming Galaxies (SFGs) produce $\gamma$ rays through the interaction of cosmic rays with the interstellar medium (see, e.g., \cite{2012ApJ...755..164A}).\FIXME{How much might they contribute, roughly?}

However, a fraction of the IGRB can originate from truly diffuse mechanisms such the interaction of Dark Matter (DM) particles. Cosmology and Astrophysics provide convincing evidences that DM constitutes 26\% of the energy content of the Universe and detecting $\gamma$ rays produced from this elusive form of matter is one of the most promising strategy to search for DM.  Since the IGRB is derived with a subtraction from LAT data of all known $\gamma$-ray mechanisms, such as the interstellar emission and flux from cataloged sources, it is one of the most promising observable to seach for a DM signature in $\gamma$ rays. \FIXME{I don't understand the logic in this last sentence.}
...
The blazar (BL Lac and FSRQ together)\FIXME{You haven't defined these terms yet.}, SFG and MAGN contribution are currently known at $20/40/50\%$ level.\FIXME{Is this relative or absolute?  I'd recommend giving the undertainties in absolute terms if possible.}
These large uncertainties reduce the sensitivity of searches targeting the DM contribution to the IGRB. 
We need to improve significantly the prediction of $\gamma$-ray emission from these sources in order use the IGRB as a promising target for the detection of DM.\FIXME{Can you give a rought estimate of how much?}
...
Below we list the different techniques that we are planning to use in order to achieve our goals.  \FIXME{I'm still not really seeing the difference between the LF / SED and the efficiency corrections.  It seems to me like the efficiency corrections is just an improved version of the LF / SED.  What am I missing?}
...
{\bf Using this more numerous sample of blazars we plan to calculate the blazar luminosity function and SED and we will be able to calculate the contribution of these sources to the IGRB with a precision of 10\%.}
...
Our research project involves the creation of a large amount of data and tools that we plan to release to the public.
These products will include the list of blazars that we will use for the determination of their luminosity function.
We plan to keep our list of blazars periodically updated with new discovered redshift measurements and with the most updated shape of their SED.
We will release also the list of SFGs and MAGN detected in $\gamma$ rays with their properties such as, classification (FRI or FRII for MAGN), redshift, and infrared or radio luminosities.
%Therefore, the comunity could use these products as the most updated list of $\gamma$-ray SFGs and MAGN.
We are planning to release also the data for the $\gamma$-ray anisotropy and flux of SFGs, MAGN and blazars. We will update these estimate once new ${\it Fermi}$-LAT catalogs will be released or new $\gamma$-ray sources will be associated to these source classes.
Finally, we will provide all the codes that we will employ in our projects with dedicated documentation.
These will include at least the program we will use to find the luminosity function of blazars, to simulate $\gamma$-ray data, to find the efficiency and the source count distribution of blazars and the set of tools that we will employ to calculate the $\gamma$-ray emission from DM.
